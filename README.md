...
| **MX Agent Notice note** | [draft-agent-notice.md](draft-agent-notice.md) | Draft (v1.0) - ensures a machine-readable HTML resource degrades to a visible, actionable signal when a lossy HTML-to-text or HTML-to-markdown fetch strips its head metadata, comments, and JSON-LD. Carries the source URL in visible text; raises to higher conformance via the `alternate` link relation and HTTP content negotiation. |
...
| **MX governance note: peer consensus and community ownership** *(governance note)* | [draft-governance-peer-consensus.md](draft-governance-peer-consensus.md) | Draft (v1.0) — why The Gathering uses community ratification rather than expert authority; the rough-consensus governance model in the W3C/IETF lineage; and why AI making expertise abundant makes peer consensus more important, not less. |
